...
{
    // Initialize libavcodec - register all codecs and formats??????????
//    av_register_all();

    // auto detect the output format from the name. default is mpeg.
    avOutputFormat = av_guess_format(nullptr, outputfile, nullptr);
    if (! avOutputFormat)
    {
        printf("Could not deduce output format from file extension: using apng.\n");
        avOutputFormat = av_guess_format("apng", nullptr, nullptr);
    }
    if (! avOutputFormat)
    {
    ...
    snprintf(avFormatContext->url, sizeof(avFormatContext->url), "%s", outputfile);
    ...
    if (video_avstream->codec == nullptr)
    {
        fprintf(stderr, "AVStream codec is nullptr\n");
        exit(1);
    }
